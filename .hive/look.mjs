...
//   const { page, errors, close } = await open({ width: 375, theme: 'dark' });
...
const base = 'http://localhost:5173/hive/';
...
async function ensureServer() {
  ...
  // Demo mode also needs VITE_DEMO_MODE, which desktops get from the untracked
  // .env.local (see is-demo-mode.ts).
...
export async function open({ width = 375, height = 812, theme = 'light' } = {}) {
  await ensureServer();
  const { chromium } = await playwright();
  const browser = await chromium.launch({ args: proxyTrust() });
  // colorScheme drives the app's System theme, the default with no hive-theme stored.
